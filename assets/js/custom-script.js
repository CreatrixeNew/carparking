...
//      $('#timepicker2').timepicker({ scrollbar: 1 ,  timeFormat: 'HH:mm:ss'});
	...
	$("#step_terminal").dynamicForm("#plus3", "#minus3", {
        limit: 5
    })
	$("#step_terminal_zone").dynamicForm("#plus4", "#minus4", {
        limit: 5
    })
	$("#step_terminal_area").dynamicForm("#plus5", "#minus5", {
        limit: 5
    })
	$("#step_terminal_bay").dynamicForm("#plus6", "#minus6", {
        limit: 5
    })
});
...
$(function()
{
function perm_all( _cbx, perm_id ){
	var checkboxes = $('#perm_'+perm_id).find(':checkbox');
    if($(_cbx).is(':checked')) {
